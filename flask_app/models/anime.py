...
class Anime:
    ...
    @classmethod
    def save_anime(cls, form):
        query = "insert into posts (content, subject, user_id) values (%(content)s, 'anime', %(user_id)s)"
        return connectToMySQL("foro_publicaciones").query_db(query, form)
    
    
    @staticmethod
    def validate_post_anime(form):
        is_valid = True
    ...
    @classmethod
    def get_anime(cls):
        query = "SELECT posts.*, users.first_name as user_name FROM posts JOIN users ON posts.user_id = users.id WHERE subject = 'anime' ORDER BY created_at DESC ;"
