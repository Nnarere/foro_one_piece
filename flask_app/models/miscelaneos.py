...
class Miscelaneos:
    ...
    @classmethod
    def save_miscelaneos(cls, form):
        query = "insert into posts (content, subject, user_id) values (%(content)s, 'miscelaneos', %(user_id)s)"
        return connectToMySQL("foro_publicaciones").query_db(query, form)
    
    @staticmethod
    def validate_post_miscelaneos(form):
        is_valid = True
    ...
    @classmethod
    def get_miscelaneos(cls):
        query = "SELECT posts.*, users.first_name as user_name FROM posts JOIN users ON posts.user_id = users.id WHERE subject = 'miscelaneos' ORDER BY created_at DESC ;"
    ...
    @classmethod
    def delete_miscelaneos(cls, data):
